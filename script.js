...
function loadPage(page){const mainContent=document.getElementById('main-content');const loadingSpinner=document.getElementById('loading-spinner');if(!pages[page])return;if(mainContent.innerHTML.trim()===pages[page].trim())return;if(page==='contact'){loadingSpinner.style.display='none'}else{loadingSpinner.style.display='block'}
console.log("Loading spinner started...");mainContent.classList.add('page-transition');setTimeout(()=>{mainContent.innerHTML=pages[page];window.scrollTo(0,0);mainContent.classList.remove('page-transition');mainContent.classList.add('page-enter');updateActiveLink(page);if(page==='portfolio'){generatePortfolioGrid();initializeModal()}
if(page==='contact'){initializeFormValidation()}
animateContentItems();setTimeout(()=>{if(page!=='contact'){loadingSpinner.style.display='none'}
console.log("Loading spinner stopped!");mainContent.classList.remove('page-enter')},300)},300)}
